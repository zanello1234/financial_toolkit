...
Estructura del CSV de Comprobantes AFIP (separado por ;):

FORMATO COMPROBANTES RECIBIDOS - COMPRAS (30 columnas):
...
Columna 7 (índice 6): Tipo Doc. Emisor
Columna 8 (índice 7): Nro. Doc. Emisor (CUIT del proveedor)
Columna 9 (índice 8): Denominación Emisor (Nombre del proveedor)
Columna 10 (índice 9): Tipo Doc. Receptor
Columna 11 (índice 10): Nro. Doc. Receptor
Columna 12 (índice 11): Tipo Cambio
Columna 13 (índice 12): Moneda
Columna 14 (índice 13): Imp. Neto Gravado IVA 0%
Columna 15 (índice 14): IVA 2,5%
Columna 16 (índice 15): Imp. Neto Gravado IVA 2,5%
Columna 17 (índice 16): IVA 5%
Columna 18 (índice 17): Imp. Neto Gravado IVA 5%
Columna 19 (índice 18): IVA 10,5%
Columna 20 (índice 19): Imp. Neto Gravado IVA 10,5%
Columna 21 (índice 20): IVA 21%
Columna 22 (índice 21): Imp. Neto Gravado IVA 21%
Columna 23 (índice 22): IVA 27%
Columna 24 (índice 23): Imp. Neto Gravado IVA 27%
Columna 25 (índice 24): Imp. Neto Gravado Total *** MONTO NETO TOTAL ***
Columna 26 (índice 25): Imp. Neto No Gravado
Columna 27 (índice 26): Imp. Op. Exentas
Columna 28 (índice 27): Otros Tributos
Columna 29 (índice 28): Total IVA
Columna 30 (índice 29): Imp. Total *** MONTO TOTAL CON IVA ***

FORMATO COMPROBANTES EMITIDOS - VENTAS (28 columnas):
Columna 1 (índice 0): Fecha de Emisión
Columna 2 (índice 1): Tipo de Comprobante
Columna 3 (índice 2): Punto de Venta
Columna 4 (índice 3): Número Desde
...
Columna 8 (índice 7): Nro. Doc. Receptor (CUIT del cliente)
Columna 9 (índice 8): Denominación Receptor (Nombre del cliente)
Columna 10 (índice 9): Tipo Cambio
Columna 11 (índice 10): Moneda
Columna 12 (índice 11): Imp. Neto Gravado IVA 0%
Columna 13 (índice 12): IVA 2,5%
Columna 14 (índice 13): Imp. Neto Gravado IVA 2,5%
Columna 15 (índice 14): IVA 5%
Columna 16 (índice 15): Imp. Neto Gravado IVA 5%
Columna 17 (índice 16): IVA 10,5%
Columna 18 (índice 17): Imp. Neto Gravado IVA 10,5%
Columna 19 (índice 18): IVA 21%
Columna 20 (índice 19): Imp. Neto Gravado IVA 21%
Columna 21 (índice 20): IVA 27%
Columna 22 (índice 21): Imp. Neto Gravado IVA 27%
Columna 23 (índice 22): Imp. Neto Gravado Total *** MONTO NETO TOTAL ***
...
Columna 28 (índice 27): Imp. Total *** MONTO TOTAL CON IVA ***
"""
    ...
    def btn_analyze_file(self):
        ...
        try:
            ...
            for i, row in enumerate(csv_reader):
                ...
                # Verificar fila completa según tipo de operación
                # Compras (comprobantes recibidos): 30 columnas
                # Ventas (comprobantes emitidos): 28 columnas
                ...
                try:
                    ...
                    # CUIT: columna 8 (índice 7) en ambos formatos
                    # Denominación: columna 9 (índice 8) en ambos formatos
                    ...
                    # Montos según tipo de operación:
                    # COMPRAS: Neto Total = índice 24 (col 25), Imp. Total = índice 29 (col 30)
                    # VENTAS: Neto Total = índice 22 (col 23), Imp. Total = índice 27 (col 28)
                    if self.operation_type == 'purchase':
                        ...
                        # Para tipo 11 (comprobantes de venta), usar total en lugar de neto
                        if doc_type_code == '11':
...
• Filas con menos de 30 columnas: {filas_cortas}
• CUITs inválidos (menos de 7 dígitos): {cuits_invalidos}
...
✅ ARCHIVO LISTO PARA PROCESAR"""
    ...
    def btn_process_file(self):
        ...
        try:
            ...
            for i, row in enumerate(csv_reader):
                ...
                # Verificar que la fila tenga suficientes columnas según tipo de operación
                # Compras (comprobantes recibidos): 30 columnas
                # Ventas (comprobantes emitidos): 28 columnas
                expected_cols = 30 if self.operation_type == 'purchase' else 28
                if len(row) < expected_cols:
                    ...
                    self._add_debug_message(f"❌ FILA {i}: Omitida por columnas insuficientes ({len(row)}<{expected_cols})")
                    continue
                ...
                try:
                    # Extraer datos básicos con limpieza mejorada
                    # CUIT y nombre están en mismo lugar en ambos formatos (índice 7 y 8)
                    cuit = row[7].strip().replace('-', '').replace(' ', '') if row[7] else ''
    ...
    def _create_invoice_simple(self, partner, row, amount, doc_type_code=None, point_of_sale=None, doc_number=None):
        ...
        try:
            ...
            # Obtener moneda según tipo de operación:
            # COMPRAS: Moneda = índice 12 (col 13)
            # VENTAS: Moneda = índice 10 (col 11)
            if self.operation_type == 'purchase':
                currency_code = row[12].strip() if len(row) > 12 else ''
            else:  # sale
            ...
            if self.import_type == 'initial_balances':
                # SALDOS INICIALES: Detectar alícuota de IVA desde el CSV
                ...
                self._add_debug_message(f"🔧 DEBUG: Línea para saldos iniciales con taxes detectados desde CSV: {tax_ids}")
            else:
    ...
    def _detect_tax_from_csv_row(self, row, doc_type_code):
        """Detectar alícuota de IVA desde los datos del CSV"""
        try:
            tax_ids = []
            
            # Índices de columnas según tipo de operación:
            # COMPRAS (30 cols): IVA 2.5%=14, IVA 5%=16, IVA 10.5%=18, IVA 21%=20, IVA 27%=22
            # VENTAS (28 cols): IVA 2.5%=12, IVA 5%=14, IVA 10.5%=16, IVA 21%=18, IVA 27%=20
            
            def parse_amount(value):
                """Convertir string a float manejando comas y puntos"""
                try:
            ...
            # Detectar qué alícuota tiene valor según tipo de operación
            if self.operation_type == 'purchase':
            ...
            self._add_debug_message(f"🔍 IVA detectado CSV: 21%={iva_21_amount}, 10.5%={iva_10_5_amount}, 27%={iva_27_amount}")
            
            # Buscar el impuesto correspondiente según la alícuota con valor mayor a 0
            if iva_27_amount > 0:
                tax = self._find_tax_by_amount(27)
                if tax:
                    tax_ids.append(tax.id)
                    self._add_debug_message(f"✅ IVA 27% detectado y configurado")
            
            if iva_21_amount > 0:
                tax = self._find_tax_by_amount(21)
                if tax:
                    tax_ids.append(tax.id)
                    self._add_debug_message(f"✅ IVA 21% detectado y configurado")
            
            if iva_10_5_amount > 0:
                tax = self._find_tax_by_amount(10.5)
                if tax:
                    tax_ids.append(tax.id)
                    self._add_debug_message(f"✅ IVA 10.5% detectado y configurado")
            
            if iva_5_amount > 0:
                tax = self._find_tax_by_amount(5)
                if tax:
                    tax_ids.append(tax.id)
                    self._add_debug_message(f"✅ IVA 5% detectado y configurado")
            
            if iva_2_5_amount > 0:
                tax = self._find_tax_by_amount(2.5)
                if tax:
                    tax_ids.append(tax.id)
                    self._add_debug_message(f"✅ IVA 2.5% detectado y configurado")
            
            # Si no se detectó ningún IVA, verificar si es tipo 11 (IVA No Corresponde)
            if not tax_ids and doc_type_code == '11':
                ...
                if tax:
                    tax_ids.append(tax.id)
                    self._add_debug_message(f"✅ IVA No Corresponde configurado para tipo 11")
            
            return tax_ids
            
        except Exception as e:
            self._add_debug_message(f"❌ Error detectando IVA desde CSV: {e}")
            return []
